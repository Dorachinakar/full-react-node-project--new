...
function editCardById(filter, card) {
  return new Promise(async (resolve, reject) => {
    const { userId, cardId } = filter;
    let user = await User.findById(userId);
    if (checkIsVip(user)) {
      visitCard
        .findOneAndUpdate({ _id: cardId }, card)
        .then((card) => {
          resolve(card);
          console.log("this is the function " + card);
        })
        .catch((err) => reject(err));
    } else {
      ...
      if (result || !result) {
        if (result) {
          ...
          card
            ...
            .then((card) => resolve(card))
            .catch((err) => reject(err));
        } else reject("error:there is card with no user,check your inputs please");
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (checkIsVip(user) || checkIsAdmin(user)) {
      await visitCard
        ...
        .then(async (card) => {
          await User.findOneAndUpdate({ _id: userId }, { $pull: { cards: card._id } })
            .then((card) => resolve(card))
            .catch((err) => reject(err));
        })
        .catch((err) => reject(err));
